...
# Step 5: Create the ggplot with error bars and significance stars
ggplot(summary_table7, aes(x = Threshold_high, y = Mean_Investment, fill = Bonus)) +
  geom_bar(stat = "identity", position = position_dodge(width = 0.9)) +
  geom_errorbar(aes(ymin = Mean_Investment - CI_95, ymax = Mean_Investment + CI_95), 
                width = 0.2, 
                position = position_dodge(width = 0.9)) +
  geom_text(
    aes(label = significance, y = Mean_Investment + CI_95 + 2),
    position = position_dodge(width = 0.9),
    vjust = 0,
    size = 5
  ) +
  ...
  theme(axis.text = element_text(size = 15), 
        ...
        axis.title = element_text(size = 15))  # Remove legend
...
# Step 2: Summarize data by Threshold_high and Bonus
summary_table7 <- data_long %>%
  group_by(Threshold_high, Bonus) %>%
  summarize(
    Mean_Investment = mean(Investment, na.rm = TRUE),
    SD_Investment = sd(Investment, na.rm = TRUE),
    n = n(),
...
# Step 3: Perform Wilcoxon test for each Bonus group
wilcox_test_results <- data_long %>%
  group_by(Bonus) %>%
  summarize(p_value = wilcox.test(Investment ~ Threshold_high)$p.value, .groups = 'drop') %>%
  mutate(
    significance = case_when(
      p_value < 0.01 ~ "***",
      p_value < 0.05 ~ "**",
...
# Step 4: Merge significance stars into the summary table
summary_table7 <- summary_table7 %>%
  left_join(wilcox_test_results, by = "Bonus")

# Step 5: Create the ggplot with error bars and significance stars
ggplot(summary_table7, aes(x = Bonus, y = Mean_Investment, fill = Threshold_high)) +
  geom_bar(stat = "identity", position = position_dodge(width = 0.9)) +
  geom_errorbar(aes(ymin = Mean_Investment - CI_95, ymax = Mean_Investment + CI_95), 
                width = 0.2, 
                position = position_dodge(width = 0.9)) +
...
# Create the ggplot with error bars
ggplot(summary_table7, aes(x = Threshold_high, y = Mean_Investment, fill=as.factor(Threshold_high))) +
  geom_bar(stat = "identity", position = position_dodge(width = 0.9)) +
  geom_errorbar(aes(ymin = Mean_Investment - CI_95, ymax = Mean_Investment + CI_95), 
                width = 0.2, 
                position = position_dodge(width = 0.9)) +
