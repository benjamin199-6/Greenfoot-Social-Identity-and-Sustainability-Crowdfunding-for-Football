...
setwd("C:/Users/rgottwei/OneDrive - WU Wien/MY DOCS/RESEARCH/Greenfoot")
...
##Baseline
regression_model <- lm(Beliefs ~ GroupType + Theme + Bonus_yes + Threshold_high, data = data_long)
...
data_long$log_Investment <- log(data_long$Investment + 1)  # Adding 1 to avoid log(0) # because of non-normality

regression_model_R <- lmer(log_Investment ~ GroupType + Theme + Bonus_yes * Threshold_high+(1|ID) , data = data_long)
tab_model(regression_model_R)

library(boot)
# Function to fit the model
fit_lmer <- function(data, indices) {
  d <- data[indices, ]  # Resample the data
  model <- lmer(Investment ~ GroupType + Theme + Bonus_yes * Threshold_high+(1|ID), data = d)
  return(fixef(model))  # Return fixed effects
}

# Perform bootstrapping
results <- boot(data = data_long, statistic = fit_lmer, R = 1000)  # 1000 resamples
...
regression_model_B <- lm(AverageGuess ~ GroupType + Theme + Bonus_yes + Threshold_high+WillingnessToTakeRisks+ Concern_ClimateChange+ Sex+HouseholdIncome + EducationLevel+InterestInFootball +
                         +SimilarityToGroupMembers +Concern_ClimateChange , data = data_long)
tab_model(regression_model_B)
...
rmanova_model <- aov(Investment ~ Bonus * Threshold_high+ Error(ID/(Bonus * Threshold_high)),data = data_long)
summary(rmanova_model)

##Assumptions
data_long <- data_long$Threshold_high %>%
  convert_as_factor(Threshold_high)

#ezANOVA
ezANOVA(
  data = data_long, dv = Investment, wid = ID,
  within = c(Bonus, Threshold_high), between = c(GroupComposition, ProjectType))

res.aov <- anova_test(
  data = data_long, dv = Investment, wid = ID,
  within = c(Bonus, Threshold_high)
)
get_anova_table(res.aov)

anova_test(data = data_long, formula = Investment ~ c(Bonus)*c(GroupComposition, ProjectType) + Error(ID/c(Bonus)))
...
ggplot(summary_table2, aes(x = Theme, y = Mean_Investment, fill = GroupType)) +
  ...
  geom_errorbar(aes(ymin = Mean_Investment - CI_95, ymax = Mean_Investment + CI_95), 
                width = 0.2, 
                position = position_dodge(width = 0.9)) +
  labs(title = "Mean Investment by Group Composition and Offsetting Reason", 
       x = "Offsetting Reason", 
       y = "Mean Investment",
       fill="Group Composition") +
  theme_minimal()+
...
ggplot(summary_table11, aes(y = Mean_Investment, x= Theme, fill=Theme)) +
  ...
  geom_errorbar(aes(ymin = Mean_Investment - CI_95, ymax = Mean_Investment + CI_95), 
                width = 0.2, 
                position = position_dodge(width = 0.9)) +
  labs(title = "Mean Investment by Offsetting Reason", 
       x = "", 
       y = "Mean Investment",
       fill= "Offsetting Reason") +
  theme_minimal()+
  scale_fill_manual(values = wes_palette("Darjeeling2"))+
  theme(axis.text=element_text(size=20), #change font size of axis text
        axis.title=element_text(size=15), #change font size of axis titles
